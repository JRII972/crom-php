...
class Partie extends DefaultDatabaseType
{
    ...
    public static function search(
        ...
        $cacheKey = self::$cachePrefix . md5(serialize([$keyword, $idJeu, $idMaitreJeu, $typePartie, $categories, $jours]));
        ...
        // Base SQL query
        $sql = 'SELECT DISTINCT p.id, p.nom, p.id_jeu, p.id_maitre_jeu, p.type_partie, p.type_campagne, 
        ...
        if ($needSessionsJoin) {
            $sql .= ' LEFT JOIN sessions s ON p.id = s.id_partie';
        }
        ...
        if ($typePartie !== '' && in_array($typePartie, [
            ...
            $sql .= ' AND p.type_partie = :type_partie';
        ...
        // Créer des objets Partie
        $parties = [];
        foreach ($results as $row) {
            try {
                $partie = new self((int)$row['id']);
                $parties[] = $partie->jsonSerialize();
            } catch (\Throwable $e) {
                continue;
            }
        }
        ...
        if (self::$cacheEnabled && extension_loaded('apcu')) {
            apcu_store($cacheKey, $parties, self::$cacheTTL);
        }
    ...
    private function invalidateCache(): void
    {
        ...
        // Invalider tous les caches de recherche des parties
