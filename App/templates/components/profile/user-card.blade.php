  ...
  <div class="card-body items-center text-center relative pb-6">
    ...
    <div class="avatar mb-4 relative">
      <div class="w-32 h-32 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
        <img src="{{ $user->profile_image }}" alt="Photo de profil" id="profile-image" />
      </div>
    ...
    {{-- Nom et prénom --}}
    <h2 class="card-title text-2xl mb-1" id="profile-full-name">{{ $user->firstname }} {{ $user->lastname }}</h2>
    
    {{-- Pseudonyme si disponible --}}
    <p class="text-base-content/70 text-lg mb-3" id="profile-pseudo">{{ $user->pseudo }}</p>
    
    {{-- Type d'utilisateur --}}
    <div class="badge badge-accent mb-4" id="profile-type">{{ $user->type }}</div>
    ...
    <div class="w-full flex flex-col gap-2 text-left mt-2">
      ...
      <div class="flex items-center gap-2">
        ...
        <span id="profile-username">login: {{ $user->username }}</span>
      </div>
      <div class="flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-primary">
        ...
        <span id="profile-birthdate">
          @if(isset($user->birthdate))
            @include('components.date-formatter', ['date' => $user->birthdate, 'format' => 'Né le j/m/Y (age ans)'])
          @endif
        </span>
      </div>
      <div class="flex items-center gap-2">
        ...
        <span id="profile-discord">Discord: {{ $user->discord }}</span>
      </div>
    </div>
