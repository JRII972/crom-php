...
<div class="card bg-base-200 shadow-xl overflow-hidden">
  ...
  <figure class="relative">
    <img src="{{ $partie->getImageURL() }}" alt="{{ $partie->getImageALT() }}" class="w-full h-60 object-cover" />
    <div class="absolute top-3 right-3">
      <div class="badge badge-lg" id="partie-type-badge">{{ $partie->getTypeFormatted() }}</div>
    </div>
    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 text-white">
      <h2 class="card-title text-2xl mb-1" id="partie-nom">{{ $partie->getNom() }}</h2>
      <p class="badge badge-sm badge-secondary opacity-80" id="partie-jeu">Jeu: {{ $partie->getJeu()->getNom() }}</p>
    </div>
  </figure>
  ...
  <div class="card-body">
    ...
    <div class="flex gap-2 mb-4" id="campagne-info">
      @if($partie->getTypeCampagne())
        <div class="badge badge-outline badge-accent" id="type-campagne-badge">{{ $partie->getTypeFormatted() }}</div>
      @endif
      <div class="badge badge-outline" id="partie-place-badge">{{ $partie->getNombreJoueursInscrits() }}/{{ $partie->getMaxJoueurs()}} places</div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-4">
      <div class="avatar">
        <div class="w-10 rounded-full">
          <img src="{{ $partie->getMaitreJeu()->getImageURL() }}" alt="Avatar du MJ" />
        </div>
      </div>
      <div>
        <p class="text-sm opacity-70">Maître du jeu</p>
        <p class="font-medium" id="partie-mj">{{ $partie->getMaitreJeu()->getNom() }} <span class="text-sm opacity-70">{{ $partie->getMaitreJeu()->getPseudonyme() }}</span></p>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2">
      <div class="flex items-center gap-2">
        ...
        <span id="partie-date-creation">Créée le {{ $partie->getDateCreation() ?? '15 Mai 2025' }}</span>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <span id="partie-joueurs-max">{{ $partie->getMaxJoueurs() ?? '5' }} joueurs maximum</span>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <span id="partie-lieu">{{ $partie->getLieu() }}</span>
      </div>
      
      <div class="flex items-center gap-2">
        <span class="material-symbols-outlined">info</span>
        <span id="partie-lieu">{{ $partie->getStatutFormatted() }}</span>
      </div>
    </div>
