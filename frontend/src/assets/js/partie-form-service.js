/**
 * Service API pour la gestion des parties
...
export class PartieFormService {
    ...
    /**
     * Charge une partie pour édition
     */
     ...
     */
    static async createPartie(formData) {
        try {
            const response = await axiosInstance.post('/parties', formData, {
                headers: {
     ...
     */
    static async updatePartie(id, formData) {
        try {
            const response = await axiosInstance.put(`/parties/${id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
    ...
    static async saveDraft(formData) {
        try {
            const response = await axiosInstance.post('/parties/draft', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
    ...
    /**
     * Validation côté client des données de partie
     */
    static validatePartieData(data) {
        const errors = [];

        // Validation du nom
        if (!data.nom || data.nom.trim().length === 0) {
            errors.push({ field: 'nom', message: 'Le nom de la partie est requis' });
        ...
        // Validation du type de partie
        const typesPartieValides = ['CAMPAGNE', 'ONESHOT', 'JEU_DE_SOCIETE', 'EVENEMENT'];
        if (!data.type_partie || !typesPartieValides.includes(data.type_partie)) {
            errors.push({ field: 'type_partie', message: 'Type de partie invalide' });
        }

        // Validation du type de campagne
        if (data.type_partie === 'CAMPAGNE' && data.type_campagne) {
...
// Export pour utilisation dans partie-form.js
window.PartieFormService = PartieFormService;
