...
const fetchCsrfToken = async () => {
  try {
    ...
    // Stocker le token dans un cookie de session, max 24 min
    if (csrfToken) {
      const expires = new Date(Date.now() + 24 * 60 * 1000).toUTCString();
      document.cookie = `csrfToken=${csrfToken}; path=/; expires=${expires}; SameSite=Strict`;
    }
    return csrfToken;
  } catch (error) {
...
function handleLoginSubmit(event) {
  ...
  apiLogin(email, password, rememberMe)
    .then(function(data) {
      if (data.status == 'success') { 
        localStorage.setItem('token', data.data.token);
        if (rememberMe && data.data.refresh_token) {
          localStorage.setItem('refreshToken', data.data.refresh_token);
        } else {
          localStorage.removeItem('refreshToken');
        }
