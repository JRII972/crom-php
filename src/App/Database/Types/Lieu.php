...
class Lieu extends DefaultDatabaseType
{
    ...
    public function __construct(
        ...
        if ($id !== null && $nom === null && $adresse === null && $latitude === null && $longitude === null && $description === null) {
            ...
            throw new InvalidArgumentException(
                'Vous devez fournir soit un ID seul, soit un nom (et éventuellement adresse, latitude, longitude, description).'
            );
        }
    }
    ...
    public function save(): void
    {
        if (isset($this->id)) {
            ...
            // Insertion
            $stmt = $this->pdo->prepare('
                INSERT INTO lieux (nom, adresse, latitude, longitude, description)
                VALUES (:nom, :adresse, :latitude, :longitude, :description)
            ');
            $stmt->execute([
                'nom' => $this->nom,
                'adresse' => $this->adresse,
    ...
    {
        $sql = 'SELECT id, nom, adresse, latitude, longitude, description FROM lieux WHERE 1=1';
        $params = [];
