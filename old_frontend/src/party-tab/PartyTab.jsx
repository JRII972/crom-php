...
import partieService from '../api/services/partieService';
...
export default function PartyTab(props) {
  ...
  React.useEffect(() => {
    let isMounted = true;
    async function fetchSessionsAndParties() {
      try {
        const data = await sessionService.listSessions();
        if (isMounted) setSessionsList(data);
      } catch {
        if (isMounted) setSessionsList([]);
      }
    }
