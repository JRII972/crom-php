...
const fetchCsrfToken = async (): Promise<string | null> => {
  try {
    const response: AxiosResponse<{ csrf_token: string }> = await axios.get(`${DOMAINE}/csrf_token`);
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const response: AxiosResponse<{ data: { token: string } }> = await axios.post(`${DOMAINE}/utilisateurs/refresh`, {
